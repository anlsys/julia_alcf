...
function test_collective_operations(comm)
    ...
    # Test Allreduce
    gpu_data = CUDA.fill(Float32(rank + 1), n)
    ...
    # Test Broadcast
    if rank == 0
        bcast_data = CUDA.fill(Float32(π), n)
    else
        bcast_data = CUDA.zeros(Float32, n)
    end
    ...
    # Test Allgather
    local_data = CUDA.fill(Float32(rank), 10)
    gathered_data = CUDA.zeros(Float32, 10 * size)
...
function test_large_data_transfer(comm)
    ...
    if rank == 0
        large_data = CUDA.rand(Float32, n)
    ...
    elseif rank == 1
        ...
        large_data_recv = CUDA.zeros(Float32, n)
...
function test_multiple_gpu_communication(comm)
    ...
    test_data = CUDA.fill(Float32(device_id + 1), 100)
...
function run_performance_benchmark(comm)
    """Run a simple performance benchmark for CUDA-aware MPI"""
    rank = MPI.Comm_rank(comm)
    size = MPI.Comm_size(comm)
    ...
    for msg_size in sizes
        data = CUDA.ones(Float32, msg_size)
        ...
        if rank == 0
            ...
            for _ in 1:10  # Average over 10 iterations
                ...
                recv_data = CUDA.zeros(Float32, msg_size)
...
function main()
    ...
    if rank == 0
        println("="^60)
        println("CUDA-aware MPI Test Suite")
        println("="^60)
        println("Number of processes: $size")
        println("Number of CUDA devices: $(CUDA.ndevices())")
        println()
    end
    ...
    try
        ...
        device_id = rank % CUDA.ndevices()
        CUDA.device!(device_id)

        if rank == 0
            println("✓ CUDA-aware MPI support confirmed")
            println()
        end
        ...
        if rank == 0
            println("\n" * "="^60)
            if all_tests_passed
                println("🎉 All CUDA-aware MPI tests PASSED!")
            else
